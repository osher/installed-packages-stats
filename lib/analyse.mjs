...
export default async ({ 
  ...
  async function analysePackage(pkg, i) {
    ...
    try {
      ...
      if (packageJsonPath.endsWith('/dist/package.json')) { //packing/transpiling error, e.g: sass-embedded/dist/package.json
        countAs('fake');
        return;
      }
      ...
      if (i) {
        ...
        if ( packageJsonPath.includes('/__tests__/')
          || packageJsonPath.startsWith(dirPath) && !packageJsonPath.slice(0, dirPath.length - 1).includes('/node_modules/')
          ) {
